...
local function BuildDiscoveriesPage(parent)
  local page=CreateFrame("Frame",nil,parent);page:SetAllPoints(parent);local title=page:CreateFontString(nil,"OVERLAY","GameFontHighlight");title:SetPoint("TOPLEFT",page,"TOPLEFT",0,0);title:SetText("Discoveries Viewer");local btnOpenViewer=CreateFrame("Button",nil,page,"UIPanelButtonTemplate");btnOpenViewer:SetSize(200,22);btnOpenViewer:SetPoint("TOPLEFT",title,"BOTTOMLEFT",0,-8);btnOpenViewer:SetText("Open Discoveries Viewer");btnOpenViewer:SetScript("OnClick",function()local Viewer=L:GetModule("Viewer",true);if Viewer and Viewer.Show then Viewer:Show()end end);local help=page:CreateFontString(nil,"OVERLAY","GameFontHighlightSmall");help:SetPoint("TOPLEFT",btnOpenViewer,"BOTTOMLEFT",0,-12);help:SetWidth(540);help:SetJustifyH("LEFT");help:SetText("Opens the comprehensive Discoveries Viewer window where you can browse, search, filter, and manage all your discoveries.");function page.refresh()end;return page
end

local function EnsureDiscoveriesPanel()
  if panel then return end; panel=CreateFrame("Frame","LootCollectorDiscoveriesPanel",InterfaceOptionsFramePanelContainer); panel.name="Discoveries"; panel.parent="LootCollector"; local title=panel:CreateFontString(nil,"OVERLAY","GameFontNormalLarge"); title:SetPoint("TOPLEFT",16,-16); title:SetText("LootCollector Discoveries"); tabs[1]=CreateTab(panel,1,"Import/Export"); tabs[2]=CreateTab(panel,2,"Discoveries"); tabs[1]:SetPoint("TOPLEFT",title,"BOTTOMLEFT",0,-8); tabs[2]:SetPoint("LEFT",tabs[1],"RIGHT",8,0); if PanelTemplates_SetNumTabs then PanelTemplates_SetNumTabs(panel,2)else panel.numTabs=2 end; if PanelTemplates_SetTab then PanelTemplates_SetTab(panel,1)end; activeTab=1; local content=CreateFrame("Frame",nil,panel); content:SetPoint("TOPLEFT",panel,"TOPLEFT",16,-88); content:SetPoint("BOTTOMRIGHT",panel,"BOTTOMRIGHT",-16,16); tabPages[1]=BuildImportExportPage(content); tabPages[2]=BuildDiscoveriesPage(content); for i,page in ipairs(tabPages)do page:ClearAllPoints(); page:SetPoint("TOPLEFT",content,"TOPLEFT",0,0); page:SetPoint("BOTTOMRIGHT",content,"BOTTOMRIGHT",0,0); page:SetShown(i==1)end; panel:SetScript("OnShow",function()local sel=(PanelTemplates_GetSelectedTab and PanelTemplates_GetSelectedTab(panel))or activeTab or 1; activeTab=sel; local page=tabPages[sel]; if page and page.refresh then page.refresh()end end); InterfaceOptions_AddCategory(panel)
end
