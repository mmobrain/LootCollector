...
DBSync.sessionIgnored = DBSync.sessionIgnored or {}
...
local function parseWire(line)
    ...
    local typ = s_match(line, "^([SDE]):")
    
    if typ == "S" then
        local sid, tot = s_match(line, "^S:([^:]+):(%d+)$")
        if sid and tot then return { t = "S", sid = sid, total = t_num(tot) or 0 } end
        return nil
        
    elseif typ == "E" then
        local sid, pk = s_match(line, "^E:([^:]+):(%d+)$")
        if sid and pk then return { t = "E", sid = sid, packets = t_num(pk) or 0 } end
        return nil
        
    elseif typ == "D" then
        local _, sid, q, rest = s_match(line, "^(D):([^:]+):(%d+):(.+)$")
        if not (sid and q and rest) then return nil end
        
        
        
        
        local iter = string.gmatch(rest .. ":", "(.-):")
        local function nextField() return iter() end
        
        local ncStr = nextField()
        if not ncStr then return nil end
        local nc = t_num(ncStr)
        ...
        local rcStr = nextField()
        if not rcStr then return nil end
        local rc = t_num(rcStr) or 0
        local recs = {}
        
        for r = 1, rc do
            local numFieldsStr = nextField()
            if not numFieldsStr then return nil end 
            local numFields = t_num(numFieldsStr) or 0
        ...
        return { t = "D", sid = sid, q = t_num(q) or 0, r = recs, n = nlist }
    end
...
function DBSync:ApplyRecord(c, z, i, x4, y4, s, fp_t0, foundBy, q, dt, it, ist, cl, itemName, sender, iz, extraFields)
    ...
    if isVendor then
        ...
        local vType = (i >= -499999 and i <= -400000) and "MS" or "BM"
        guid = L:GenerateVendorGUID(vType, c, z, iz_val, x, y)
    else
        target_db = L:GetDiscoveriesDB()
        if not target_db then return end
        guid = L:GenerateGUID(c, z, iz_val, i, x, y)
    end
    ...
    if not existing then
        ...
        if Constants then
            if dt == Constants.DISCOVERY_TYPE.MYSTIC_SCROLL then CLUSTER_YARDS = Constants.CLUSTER_YARDS_MS or 200
            elseif dt == Constants.DISCOVERY_TYPE.BLACKMARKET then CLUSTER_YARDS = Constants.CLUSTER_YARDS_VEND or 20
            else CLUSTER_YARDS = Constants.CLUSTER_YARDS_WF or 100 end
        end

        for g, ex in pairs(target_db) do
            if isVendor then
                if ex.vendorName == itemName and ex.c == c and ex.z == z then
                    local dist = L:ComputeDistance(c, z, x, y, ex.c, ex.z, ex.xy.x, ex.xy.y)
    ...
    if not existing then
        ...
        local newRecord = {
            ...
            ac = math.random(2, 4),
            o  = sender, fp = foundBy,
            q  = finalQ or 0, dt = dt or 0, it = it or 0, ist = ist or 0, cl = cl,
...
function DBSync.Shares(scope, targetOrZone, zoneId)
    ...
    local totalSent = 0
    local limit = DBSync.MAXMSG or 250
...
StaticPopupDialogs["LOOTCOLLECTOR_DBSYNC_CONFIRM"] = {
    ...
    OnCancel = function(selfFrame, data, reason)
        local sid = data and data.sid
        local sender = data and data.sender
        if reason == "clicked" then 
            if sender then DBSync.sessionIgnored[sender] = true end
            print(string.format("|cffff7f00LootCollector:|r Ignoring database shares from %s for this session.", sender or "unknown"))
        end
        if sid then DBSync:ClearSidBuffer(sid, true) end
    end,
    OnAlt = function(selfFrame, data) 
        local sender = data and data.sender
        local sid = data and data.sid
        if sender then
            ...
            DBSync.sessionIgnored[sender] = true
            print(string.format("|cffff0000LootCollector:|r Player %s permanently blocked.", sender))
        end
        if sid then DBSync:ClearSidBuffer(sid, true) end
    end,
...
function DBSync:OnCommReceived(prefix, message, distribution, sender)
    ...
    if self.sessionIgnored[sender] then 
        dprint("RX drop: session ignored")
        return 
    end
        ...
        if exp then
			...
			if not anyActiveForSender then
                    ...
                    if total and total > 0 then
                        print(string.format("|cff00ff00LootCollector|r: Finished importing %d records from %s. Running spatial cleanup...", total, sender))
                        
                      
                        local Core = L:GetModule("Core", true)
                        if Core and Core.DeduplicateItems then
                            local wfRem, msRem = Core:DeduplicateItems(true)
                            if wfRem > 0 or msRem > 0 then
                                print(string.format("|cff00ff00LootCollector|r: Cleanup removed %d duplicate WF and %d duplicate MS items from import.", wfRem, msRem))
                            end
