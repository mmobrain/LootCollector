...
local function buildExport(includeOverlays, includeTombstones)
    ...
    if includeTombstones then
        if L.db and L.db.global and L.db.global.deletedCache then
            tombstones = {}
            local tnow = time()
            for key, data in pairs(L.db.global.deletedCache) do
                
                local isExpired = false
                if data.expiresAt and tnow > data.expiresAt then isExpired = true
                elseif data.deletedAt and (tnow - data.deletedAt) > (90 * 86400) then isExpired = true end
    ...
    local aoeTombstones = nil
    if includeTombstones then
        if L.db and L.db.global and L.db.global.aoeTombstones then
            aoeTombstones = {}
            local tnow = time()
            for _, data in ipairs(L.db.global.aoeTombstones) do
                
                if not data.expires or tnow <= data.expires then
                    table.insert(aoeTombstones, {
                        name = data.name,
                        c = data.c, z = data.z, iz = data.iz, dt = data.dt,
                        x = data.x, y = data.y,
                        radius = data.radius,
    ...
    return {
        meta = {
            ...
            counts = {
                ...
                tombstones = tombstones and countTableKeys(tombstones) or 0,
                aoeTombstones = aoeTombstones and #aoeTombstones or 0,
            },
        },
...
function ImportExport:ExportString(includeOverlays, includeTombstones)
	local payload = buildExport(includeOverlays, includeTombstones)
...
function ImportExport:ApplyImport(parsed, mode, withOverlays, skipBlacklist, skipWhitelist, isStarterDB)
	...
	for oldGuid, d in pairs(disc) do
	    local isCoAIncompatible = false
	    if isCoA then
	        local dt = tonumber(d.discoveryType) or 0
	        local ist = tonumber(d.itemSubType) or 0
	        if dt == MS_TYPE or ist == 6 or ist == 7 then
	            isCoAIncompatible = true
	        end
	    end
	
	    if isCoAIncompatible then
	        applied.skippedCoA = applied.skippedCoA + 1
        elseif Constants and Constants.IsForbiddenZone and Constants:IsForbiddenZone(d.continent, d.zoneID, d.foundBy_player) then
            ...
            if existing then
                ...
                if imp_t0 < (tonumber(existing.t0) or currentTime) then
                    existing.t0 = imp_t0
                end
                
                existing.fp_votes = existing.fp_votes or {}
                if d.fp_votes then
                    for voter, vData in pairs(d.fp_votes) do
                        if not existing.fp_votes[voter] then
                            existing.fp_votes[voter] = { score = vData.score, t0 = vData.t0 }
                        else
                            existing.fp_votes[voter].score = existing.fp_votes[voter].score + vData.score
                            if vData.t0 < existing.fp_votes[voter].t0 then
                                existing.fp_votes[voter].t0 = vData.t0
                            end
                        end
                    end
                elseif d.foundBy_player then
                    if not existing.fp_votes[d.foundBy_player] then
                        existing.fp_votes[d.foundBy_player] = { score = 1, t0 = imp_t0 }
                    else
                        existing.fp_votes[d.foundBy_player].score = existing.fp_votes[d.foundBy_player].score + 1
                    end
                end
                
                if Core and Core.UpdateConsensusWinner then
                    Core:UpdateConsensusWinner(existing)
                end
                ...
                shortRecord.mid = L:ComputeCanonicalDiscoveryMid(shortRecord)
    ...
    for oldGuid, d in pairs(bm_vendors) do
        local vType = d.vendorType
        if not vType then
            local i = tonumber(d.itemID) or 0
            vType = (i >= -499999 and i <= -400000) and "MS" or "BM"
        end
        
        local isCoAIncompatible = false
        if isCoA and vType == "MS" then
            isCoAIncompatible = true
        end
        ...
        elseif Constants and Constants.IsForbiddenZone and Constants:IsForbiddenZone(d.continent, d.zoneID, d.foundBy_player) then
            ...
            local newGuid = L:GenerateVendorGUID(vType, d.continent, d.zoneID, iz, x, y)

            local existing, existingGuid = bm_db[newGuid], newGuid
            if not existing then
                for g, ex in pairs(bm_db) do
                    if ex.vendorName == d.vendorName and ex.c == d.continent and ex.z == d.zoneID then
                        local dist = L:ComputeDistance(d.continent, d.zoneID, x, y, ex.c, ex.z, ex.xy.x, ex.xy.y)
                        local radius = GetClusterRadius(ex.dt)
                        if dist and dist <= radius then
                            existing = ex
            ...
            if existing then
                ...
                if d.vendorItems and #d.vendorItems > 0 then
                    ...
                    for _, item in ipairs(d.vendorItems) do
	...
	if withOverlays then
		if L.db and L.db.char and parsed.overlays then
			...
			for oldGuid, on in pairs(parsed.overlays.hidden or {}) do
				if on then 
                    local mappedGuid = guidMap[oldGuid] or oldGuid
                    L.db.char.hidden[mappedGuid] = true 
                end
			end
		end
    end
    ...
    if parsed.tombstones then
        L.db.global.deletedCache = L.db.global.deletedCache or {}
        local importedTombstones = 0
        local tnow = time()
        for key, data in pairs(parsed.tombstones) do
            local existing = L.db.global.deletedCache[key]
            if not existing or (data.expiresAt and (not existing.expiresAt or data.expiresAt > existing.expiresAt)) then
                L.db.global.deletedCache[key] = data
    ...
    if parsed.aoeTombstones then
        L.db.global.aoeTombstones = L.db.global.aoeTombstones or {}
        local importedAOE = 0
        local tnow = time()
        
        for _, importedTS in ipairs(parsed.aoeTombstones) do
            
            
            if importedTS.name and type(importedTS.name) == "string" and importedTS.name:match("^LocalCluster") then
                importedTS.expires = tnow + (10 * 86400)
            end
            
            if not importedTS.expires or tnow <= importedTS.expires then
                local found = false
                for _, localTS in ipairs(L.db.global.aoeTombstones) do
                    if localTS.name and localTS.name == importedTS.name then
                        if importedTS.expires and (not localTS.expires or importedTS.expires > localTS.expires) then
                            localTS.c, localTS.z, localTS.iz, localTS.dt = importedTS.c, importedTS.z, importedTS.iz, importedTS.dt
                            localTS.x, localTS.y = importedTS.x, importedTS.y
                            localTS.radius = importedTS.radius
                            localTS.expires = importedTS.expires
                            importedAOE = importedAOE + 1
                        end
                        found = true
                        break
                    end
                ...
                if not found then
                    table.insert(L.db.global.aoeTombstones, importedTS)
                    importedAOE = importedAOE + 1
                end
            end
        end
        ...
        if importedAOE > 0 then
            ...
            if Core and Core.GetDiscoveriesDB then
                ...
                if dbDisc then
                    local aoeGuidsToRemove = {}
                    for guid, d in pairs(dbDisc) do
                        if Core.IsInsideAOETombstone and Core:IsInsideAOETombstone(d.c, d.z, d.iz, d.dt, d.xy and d.xy.x or 0, d.xy and d.xy.y or 0) then
                            table.insert(aoeGuidsToRemove, guid)
                        end
                    end
...
local function OpenExportDialog(includeOverlays, includeTombstones)
	...
	local s, err = ImportExport:ExportString(includeOverlays, includeTombstones)
...
local function OpenImportDialog()
	...
	if not f.isRedesigned then
		...
		f.btnImportFile:SetScript("OnClick", function()
		    ...
		    local loaded, reason = LoadAddOn("LootCollector_CustomImport")
		    ...
		    if loaded then
			    if type(_G.LootCollector_CustiomImport) == "string" and _G.LootCollector_CustiomImport ~= "" then
				    ...
				    print("|cffff7f00LootCollector:|r The 'customimport.lua' file is empty. Please add an import string to it.")
			    end
			else
			    print(string.format("|cffff7f00LootCollector:|r Custom import file could not be loaded. Reason: %s", tostring(reason)))
			end
		end)
...
local function BuildImportExportPage(parent)
	...
	local bDev = CreateFrame("Button", nil, p, "UIPanelButtonTemplate")
	bDev:SetSize(120, 22)
	bDev:SetPoint("LEFT", b3, "RIGHT", 8, 0)
	bDev:SetText("Export with TS")
	bDev:SetScript("OnClick", function() 
	    
	    OpenExportDialog(false, true) 
	end)
	
	local DevModule = L:GetModule("DevCommands", true)
	if not DevModule then
	    bDev:Hide()
	end
	...
	btnMergeStarter:SetScript("OnClick", function()
		...
		if _G.LootCollector_OptionalDB_Data and _G.LootCollector_OptionalDB_Data.data then
			...
			print(string.format("|cffff7f00LootCollector:|r Starter database could not be loaded. Reason: %s", tostring(reason)))
		end
	end)
	...
	local purgeAOEBtn = CreateFrame("Button", nil, p, "UIPanelButtonTemplate")
	    purgeAOEBtn:SetSize(130, 22)
	    purgeAOEBtn:SetPoint("LEFT", btnOverrideStarter, "RIGHT", 8, 0)
	    purgeAOEBtn:SetText("Purge AOE TS")
	    purgeAOEBtn:SetScript("OnClick", function()
        if L.db and L.db.global and L.db.global.aoeTombstones then
            local tsList = L.db.global.aoeTombstones
            ...
            for i = #tsList, 1, -1 do
                ...
                if ts.name and type(ts.name) == "string" and ts.name:match("^LocalCluster") then
                    table.remove(tsList, i)
                    removedCount = removedCount + 1
                end
            end
            
            if removedCount > 0 then
                print(string.format("|cff00ff00LootCollector:|r Purged %d auto-generated local Deadzones. A /reload may be required for surrounding pins to reappear.", removedCount))
            else
                print("|cffff7f00LootCollector:|r No auto-generated local Deadzones found to purge.")
            end
        end
    end)
...
local function BuildListPage(parent, titleText, dataFilterFunc)
	...
	if titleText == "All World Discoveries" then
		...
		local purgeDelCacheBtn = CreateFrame("Button", nil, page, "UIPanelButtonTemplate")
		purgeDelCacheBtn:SetSize(140, 24)
		purgeDelCacheBtn:SetPoint("RIGHT", purgeBtn, "LEFT", -8, 0)
		purgeDelCacheBtn:SetText("Purge Deleted Cache")
		purgeDelCacheBtn:SetScript("OnClick", function()
		    if L.db and L.db.global and L.db.global.deletedCache then
		        wipe(L.db.global.deletedCache)
		        print("|cff00ff00LootCollector:|r Deleted Cache (single-item tombstones) wiped successfully.")
		    end
		end)		
		
        local nukeBtn = CreateFrame("Button", nil, page, "UIPanelButtonTemplate")
		nukeBtn:SetSize(120, 24)
		nukeBtn:SetPoint("RIGHT", purgeAOEBtn, "LEFT", -8, 0)
...
function ImportExport:ShowReportGenerator()
    if not self.reportGenWindow then
        local f = CreateFrame("Frame", "LootCollectorReportGenerator", UIParent)
        f:SetSize(500, 520)
        f:SetPoint("CENTER")
        f:SetFrameStrata("DIALOG")
        f:SetMovable(true)
	  ...
	  f:SetBackdropColor(0,0,0,1)
        ...
        f.title:SetText("Generate Bug Report")

        
        local titleLabel = f:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        titleLabel:SetPoint("TOPLEFT", 24, -45)
        titleLabel:SetText("Report Title (Brief summary):")

        f.titleBox = CreateFrame("EditBox", nil, f, "InputBoxTemplate")
        f.titleBox:SetSize(440, 20)
        f.titleBox:SetPoint("TOPLEFT", titleLabel, "BOTTOMLEFT", 6, -5)
        f.titleBox:SetAutoFocus(false)
        f.titleBox:SetMaxLetters(100)

        
        local descLabel = f:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        descLabel:SetPoint("TOPLEFT", titleLabel, "BOTTOMLEFT", 0, -40)
        descLabel:SetText("Description (Steps to reproduce, what happened):")
        
        local helperLabel = f:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
        helperLabel:SetPoint("TOPLEFT", descLabel, "BOTTOMLEFT", 0, -4)
        helperLabel:SetText("Note: Settings, addon list, and metrics are attached automatically.")
        helperLabel:SetTextColor(0.6, 0.6, 0.6)

        local scrollFrame = CreateFrame("ScrollFrame", nil, f, "UIPanelScrollFrameTemplate")
        scrollFrame:SetPoint("TOPLEFT", helperLabel, "BOTTOMLEFT", 6, -10)
        scrollFrame:SetSize(420, 220)
        
        local bg = CreateFrame("Frame", nil, scrollFrame, "BackdropTemplate")
        bg:SetPoint("TOPLEFT", -6, 4)
        bg:SetPoint("BOTTOMRIGHT", 26, -4)
        bg:SetBackdrop({ bgFile="Interface\\ChatFrame\\ChatFrameBackground", edgeFile="Interface\\Tooltips\\UI-Tooltip-Border", tile=true, tileSize=16, edgeSize=16, insets={left=4,right=4,top=4,bottom=4} })
        bg:SetBackdropColor(0.05, 0.05, 0.05, 1)
        bg:SetFrameLevel(scrollFrame:GetFrameLevel() - 1)
        
        
        local focusCatcher = CreateFrame("Button", nil, bg)
        focusCatcher:SetAllPoints()
        focusCatcher:SetScript("OnClick", function()
            if f.descBox then f.descBox:SetFocus() end
        end)

        f.descBox = CreateFrame("EditBox", nil, scrollFrame)
        f.descBox:SetMultiLine(true)
        f.descBox:SetFontObject(ChatFontNormal)
        f.descBox:SetWidth(410)
        f.descBox:SetAutoFocus(false)
        f.descBox:SetScript("OnEscapePressed", function(self) self:ClearFocus() end)
        scrollFrame:SetScrollChild(f.descBox)

        
        local instrBox = CreateFrame("Frame", nil, f, "BackdropTemplate")
        instrBox:SetPoint("TOPLEFT", scrollFrame, "BOTTOMLEFT", -6, -15)
        instrBox:SetPoint("BOTTOMRIGHT", f, "BOTTOMRIGHT", -24, 50)
        instrBox:SetBackdrop({ bgFile="Interface\\ChatFrame\\ChatFrameBackground", edgeFile="Interface\\Tooltips\\UI-Tooltip-Border", tile=true, tileSize=16, edgeSize=16, insets={left=4,right=4,top=4,bottom=4} })
        instrBox:SetBackdropColor(0.04, 0.15, 0.04, 1)
        
        local instrText = instrBox:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
        instrText:SetPoint("TOPLEFT", 10, -10)
        ...
        f.btnClose = CreateFrame("Button", nil, f, "UIPanelButtonTemplate")
        f.btnClose:SetSize(100, 22)
        f.btnClose:SetPoint("BOTTOMRIGHT", -20, 20)
        f.btnClose:SetText("Cancel")
        f.btnClose:SetScript("OnClick", function() f:Hide() end)
        
        f.btnGenerate = CreateFrame("Button", nil, f, "UIPanelButtonTemplate")
        f.btnGenerate:SetSize(160, 22)
        f.btnGenerate:SetPoint("RIGHT", f.btnClose, "LEFT", -10, 0)
        f.btnGenerate:SetText("|cff00ff00Generate Payload|r")
        f.btnGenerate:SetScript("OnClick", function()
            local titleText = f.titleBox:GetText()
            local descText = f.descBox:GetText()
            if titleText == "" then titleText = "Untitled Report" end
            if descText == "" then descText = "No description provided." end
            f:Hide()
            ImportExport:ExportDebugData(titleText, descText)
        end)
        
        self.reportGenWindow = f
    end
    
    self.reportGenWindow.titleBox:SetText("")
    self.reportGenWindow.descBox:SetText("")
    self.reportGenWindow:Show()
end

function ImportExport:ExportDebugData(reportTitle, reportDesc)
    local payload = {}
    ...
    local Constants = L:GetModule("Constants", true)
    payload.env = {
        version = L.Version or "Unknown",
        playerName = UnitName("player") or "Unknown",
        realm = L.GetActiveRealmKey and L:GetActiveRealmKey() or (GetRealmName() or "Unknown"),
        mode = Constants and Constants.ACTIVE_REALM_TYPE or "Unknown",
        playerClass = select(2, UnitClass("player")),
        playerLevel = UnitLevel("player"),
        mapC = GetCurrentMapContinent() or 0,
        mapZ = GetCurrentMapAreaID() or 0,
    }
    ...
    for i = 1, GetNumAddOns() do
        ...
        if loadable or reason == "DEMAND_LOADED" then
            table.insert(payload.addons, name)
        end
    end
    ...
    if L.db and L.db.profile then
        for k, v in pairs(L.db.profile) do
            if k ~= "favorites" and k ~= "invalidSenders" then
    ...
    local cDisc, cVend, cLoot, cTomb = 0, 0, 0, 0
    local wfCount, msCount = 0, 0
    
    local dbDisc = L:GetDiscoveriesDB()
    local dbVend = L:GetVendorsDB()
    
    if dbDisc then 
        for _, d in pairs(dbDisc) do 
            cDisc = cDisc + 1 
            if Constants then
                if d.dt == Constants.DISCOVERY_TYPE.WORLDFORGED then wfCount = wfCount + 1
                elseif d.dt == Constants.DISCOVERY_TYPE.MYSTIC_SCROLL then msCount = msCount + 1 end
            end
        end 
    end
    if dbVend then for _ in pairs(dbVend) do cVend = cVend + 1 end end
    if L.db and L.db.char and L.db.char.looted then for _ in pairs(L.db.char.looted) do cLoot = cLoot + 1 end end
    if L.db and L.db.global and L.db.global.deletedCache then for _ in pairs(L.db.global.deletedCache) do cTomb = cTomb + 1 end end
    ...
    payload.profiler = L._profilerStats or {}
    
    local ok, serialized = pcall(AceSerializer.Serialize, AceSerializer, payload)
    if ok and serialized then
        local compressed = LibDeflate:CompressDeflate(serialized, {level = 9})
        local encoded = LibDeflate:EncodeForPrint(compressed)
        local finalString = "!LCDBG1!" .. encoded
        
        if not ImportExport.exportDialog then
            ImportExport.exportDialog = CreateEditDialog("LootCollectorExportDialog", "LootCollector Export", true)
        end
        ImportExport.exportDialog.title:SetText("LootCollector Debug Payload Generated")
        ImportExport.exportDialog.edit:SetText(finalString)
        ImportExport.exportDialog:Show()
        ImportExport.exportDialog.edit:HighlightText()
        ImportExport.exportDialog.edit:SetFocus()
    else
        print("|cffff0000LootCollector:|r Failed to generate debug payload.")
    end
end
