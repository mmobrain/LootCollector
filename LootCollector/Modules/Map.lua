...
Map._reusableVisibleDiscoveries = {}
...
function Map:FocusOnDiscovery(d)
    ...
    local targetGuid = type(d) == "string" and d or d.g
    if not targetGuid then return end

    
    local targetZ = nil
    if type(d) == "table" and d.z then
        targetZ = tonumber(d.z)
    else
        
        local parts = { strsplit("-", targetGuid) }
        if tonumber(parts[1]) then
            
            targetZ = tonumber(parts[2])
        else
            
            targetZ = tonumber(parts[3])
        end
    end

    L._mdebug("Map-Focus", string.format("FocusOnDiscovery initiated. Target GUID: %s | Target Zone: %s", tostring(targetGuid), tostring(targetZ)))
    ...
    if targetZ and targetZ > 0 and SetMapByID then
        SetMapByID(targetZ - 1)
        L._mdebug("Map-Focus", "Switched map to zone ID: " .. tostring(targetZ))
    elseif SetMapToCurrentZone then
        SetMapToCurrentZone()
        L._mdebug("Map-Focus", "SetMapByID failed or targetZ missing. Fell back to current zone.")
    end
    ...
    local function TryFocus()
        ...
        for _, pin in ipairs(Map.pins) do
            if pin:IsShown() and pin.discovery and pin.discovery.g == targetGuid then
                targetPin = pin
                break
            end
        end
        ...
        if targetPin then
            L._mdebug("Map-Focus", "Pin found on map! Executing animation overlay.")
            ...
            local dbRec = L:GetDiscoveriesDB()[targetGuid] or L:GetVendorsDB()[targetGuid]
            local sourceForIcon = dbRec or (type(d) == "table" and d) or nil
            local iconTexture = sourceForIcon and Map:GetDiscoveryIcon(sourceForIcon) or PIN_FALLBACK_TEXTURE
            
            overlayPin.itemIcon:SetTexture(iconTexture)
            overlayPin:Show()
            ...
            if attempts < 10 then 
                ...
                L._mdebug("Map-Focus", "Could not find target map pin for GUID: " .. tostring(targetGuid) .. " after retries.")
            end
        end
    end
...
function Map:OnInitialize()
    ...
    local function CheckMapReadiness()
        ...
        if (px and py and (px > 0 or py > 0)) and (mapID and mapID > 0) then
            ...
            Map._cachedLocation.mapID = mapID
            Map._cachedLocation.px = px
            Map._cachedLocation.py = py

            if not Map.mapSystemReady then
                L._mdebug("Map-Init", "Map system is now READY. Valid coords detected.")
    ...
    self:RegisterEvent("PLAYER_ENTERING_WORLD", function()
        ...
        ticker = C_Timer.NewTicker(0.5, function()
            ...
            elseif attempts > 20 then
                ticker:Cancel()
                L._mdebug("Map-Init", "Map readiness check timed out. Forcing ready.")
                Map.mapSystemReady = true
                if Map.UpdateMinimap then Map:UpdateMinimap() end
            end
        end)
...
function Map:GetPlayerLocation()
    local cache = self._cachedLocation
    
    if WorldMapFrame and WorldMapFrame:IsVisible() then
        
        
        local viewedC = GetCurrentMapContinent()
        local viewedZ = GetCurrentMapAreaID()
        
        if viewedC == cache.c and viewedZ == cache.mapID then
...
function Map:RebuildFilteredCache()
    ...
    if zoneGUIDs then
        for _, guid in ipairs(zoneGUIDs) do
            local d = (discoveries and discoveries[guid]) or (vendors and vendors[guid])
            if d then
                local isContinentOrWorld = (d.c == 0 and d.z == 0) or (d.c == 1 and d.z == 14) or (d.c == 2 and d.z == 15)
    ...
    L._mdebug("Map-Cache", "Minimap Cache rebuilt locally. Total passing filters: " .. #self.cachedVisibleDiscoveries)
    self.cacheIsDirty = false
...
local function BuildFilterEasyMenu()
  ...
  addToggle("Hide Uncached", "hideUncached", hideSub, false)
    ...
    FilterButton:SetScript("OnClick", function(self)
        ...
        if spaceAbove > estimatedMenuHeight then
            ...
            if dropdown then
                ...
                dropdown:SetPoint("BOTTOMRIGHT", self, "TOPRIGHT", 0, 4)
            end
        else
            EasyMenu(menu, FilterMenuHost, self, 0, 0, "MENU", 2)
            if dropdown then
                dropdown:ClearAllPoints()
                dropdown:SetPoint("TOPRIGHT", self, "BOTTOMRIGHT", 0, -4)
            end
        end
    end)
...
function Map:BuildPin()
  ...
  frame:SetScript("OnEnter", function(self)
    ...
    if IsAltKeyDown() or isVendor then
        L._mdebug("Map", "OnEnter - Pin (ALT pressed or Vendor, showing detailed tooltip)")
...
function Map:UpdateMinimap()
    ...
    local f = L:GetFilters()    
    if not f or not f.showMinimap or f.hideAll or L:IsPaused() or not Minimap or (L.IsZoneIgnored and L:IsZoneIgnored()) then
        self:HideAllMmPins()
        self:StopMinimapTicker()
        if pTime then L:ProfileStop("Map:UpdateMinimap", pTime) end 
        return
    end
    ...
    local visibleDiscoveries = self._reusableVisibleDiscoveries
    wipe(visibleDiscoveries)
    ...
    for _, d in ipairs(sourceList) do
        ...
        if not isContinentOrWorld then
            local shouldShow = false
            local iz = tonumber(d.iz) or 0
            local isVisibleOnThisMap = (iz == 0 or iz == currentMapID)
            
            if isVisibleOnThisMap then
                if self.cachingEnabled then
    ...
    for i = 1, math.max(#self._mmPins, #visibleDiscoveries) do
        ...
        if discovery then
            ...
            if discovery.s == "STALE" then
                pin.tex:SetVertexColor(0.9, 0.5, 0.0)
                pin.color_frame:Show()
                pin.color_frame:SetVertexColor(1, 1, 1)
            ...
            pin:SetAlpha(AlphaForStatus(L:GetDiscoveryStatus(discovery)))
        else
            pin:Hide()
            pin.discovery = nil
        end
    ...
    self._mmTicker:SetScript("OnUpdate", function(_, elapsed)
        ...
        if Map._mmElapsed >= Map._mmInterval then
            ...
            local playerMoved = not (
                ...
                m_abs(px - (state.px or -1)) < 0.0001 and
                m_abs(py - (state.py or -1)) < 0.0001 and
                m_abs(facing - (state.facing or -1)) < 0.001
            )
                    ...
                    if d.c == c and d.z == mapID then
                        ...
                        if distYards and xDist and yDist then
                            if maxDistSq and (distYards * distYards) > maxDistSq then
                                ...
                                local dist
                                if useCircular then
                                    dist = m_sqrt(xDist * xDist + yDist * yDist)
                                else
                                    dist = m_max(m_abs(xDist), m_abs(yDist))
                                end
...
function Map:OpenShowToDialog(discovery)
  ...
  L._mdebug("Map-ShowTo", string.format("Opening 'Show To' dialog. Discovery dt: %s, src: %s", tostring(discovery.dt), tostring(discovery.src)))
...
function Map:RefreshPinIconsForItem(itemID)
    ...
    for _, pin in ipairs(self.pins) do
        if pin and pin.discovery and pin.discovery.i == itemID then
            L._mdebug("Map-Cache", "Found matching pin to refresh.")
            local d = pin.discovery
...
function Map:DrawWorldMapPins()
    ...
    if not WorldMapDetailFrame or not WorldMapButton then 
        if pTime then L:ProfileStop("Map:DrawWorldMapPins", pTime) end
        return 
    end
...
function Map:AddOrUpdatePinV5(rec)
  ...
  local isContinentOrWorld = (c == 0 and z == 0) or (c == 1 and z == 14) or (c == 2 and z == 15)
  if isContinentOrWorld then return false end
  ...
  if c == currentContinent and z == currentZoneID then
      ...
      if iz > 0 and iz ~= currentZoneID then
          return false
      end
      self:Update()
      return true
