...
local function makeItemDisplay(d)
    ...
    if pTime then L:ProfileStop("Toast:makeItemDisplay", pTime) end 
...
local function makeTickerLineFromDiscovery(d, options)
    ...
    local op = options.op or "DISC"
    local hideNames = L.db and L.db.profile and L.db.profile.hidePlayerNames

    local result
    if hideNames or op == "CONF" then
        result = string.format("%s in %s!", itemDisplay, tostring(zoneName))
    else
	    result = string.format("%s in %s found by %s!", itemDisplay, tostring(zoneName), tostring(finder))
    end
    
    if pTime then L:ProfileStop("Toast:makeTickerLineFromDiscovery", pTime) end 
    return result
end
...
local function checkQueueForSpam()
	...
	if L.ANTISPAMDISABLED then 
        if pTime then L:ProfileStop("Toast:checkQueueForSpam", pTime) end
        return 
    end
	if #queue < SPAMCHECKQUEUEMIN then 
        if pTime then L:ProfileStop("Toast:checkQueueForSpam", pTime) end
        return 
    end
...
local function removeFirstNFromQueue(n)
    ...
    if n <= 0 then 
        if pTime then L:ProfileStop("Toast:removeFirstNFromQueue", pTime) end
        return 
    end        
    local total = #queue
    for i = 1, total - n do
...
function Toast:ApplySettings()
	...
	if ticker and ticker.texts then
        for i = 1, 2 do
            local fs = ticker.texts[i]
            if fs then
                local fnt, curSize = fs:GetFont()
                if not ticker.baseFontSize then
                    local _, bs = fs:GetFont()
                    ticker.baseFontSize = bs or 10
                end
                local flags = (self.tickerOutline and TICKERFONTFLAGS or "")
                fs:SetFont(fnt, ticker.baseFontSize + self.tickerFontDelta, flags)
...
function Toast:acquireToast()
	...
	for _, t in ipairs(toasts) do
		if not t:IsShown() then 
            if pTime then L:ProfileStop("Toast:acquireToast", pTime) end
            return t 
        end
	end
...
local function renderToast(d, options)
	...
	ensureProfile()
	if not (L.db and L.db.profile and L.db.profile.toasts and L.db.profile.toasts.enabled) then
        if pTime then L:ProfileStop("Toast:renderToast", pTime) end
		return
	end
...
local function tickerEnsureChildSize()
    ...
    for i = 1, 2 do
        if ticker.texts[i] then
            ticker.texts[i]:SetHeight(sh)
            ticker.texts[i]:SetJustifyV("MIDDLE")
        end
    end
end

local function tickerLoadNextMessage(fontStringIndex)
    local fs = ticker.texts[fontStringIndex]
    if not fs then return false end
    
    if #ticker.messages == 0 then
        fs:SetText("")
        return false
    end
    ...
    local sw = ticker.scrollContainer:GetWidth() or 0
    fs.targetWidth = fs:GetStringWidth() + (sw * 0.2) 
...
local function tickerBuildSessionLine()
	...
	if not ticker.ready then
		ticker.text:SetText("")
		ticker.textX = 0.0
        if pTime then L:ProfileStop("Toast:tickerBuildSessionLine", pTime) end
		return
	end
...
local function addToTicker(d, options)
    ...
    if pname and spamState.blocked[pname] then 
        if pTime then L:ProfileStop("Toast:addToTicker", pTime) end
        return 
    end
    ...
    if not ticker.active then
        ...
        tickerEnsureChildSize()
        tickerLoadNextMessage(1)
        tickerLoadNextMessage(2)
        
        ticker.activeStringIndex = 1
        ticker.scrollStartTime = GetTime()
...
local function enqueueToast(d, options)
    ...
    if pTime then L:ProfileStop("Toast:enqueueToast", pTime) end 
end
...
local function processAnonWindow()
	...
	if not anonWindowEnds then 
        if pTime then L:ProfileStop("Toast:processAnonWindow", pTime) end
        return 
    end
	if GetTime() < anonWindowEnds then 
        if pTime then L:ProfileStop("Toast:processAnonWindow", pTime) end
        return 
    end
	anonWindowEnds = nil
	if #anonBuffer == 0 then 
        if pTime then L:ProfileStop("Toast:processAnonWindow", pTime) end
        return 
    end
...
function Toast:Show(discoveryData, force, options)
	...
	if toastOnlyNew and not isNewRecord then
        if pTime then L:ProfileStop("Toast:Show", pTime) end
	    return
	end
	
	local minQuality = tonumber(L.db.profile.toasts.toastMinQuality) or 2
	local itemQuality = tonumber(discoveryData and discoveryData.q) or 1
...
function Toast:OnInitialize()
    ...
    for i = 1, 2 do
        ticker.texts[i] = ticker.child:CreateFontString(nil, "ARTWORK", "GameFontNormalSmall")
        local fs = ticker.texts[i]
        local fnt, size = fs:GetFont()
        if i == 1 then ticker.baseFontSize = size or 10 end
        local flags = (Toast.tickerOutline and TICKERFONTFLAGS or "")
        fs:SetFont(fnt, ticker.baseFontSize + Toast.tickerFontDelta, flags)
        fs:SetJustifyH("LEFT")
        fs:SetJustifyV("MIDDLE")
    ...
    dispatcher:SetScript("OnUpdate", function(_, elapsed)
        ...
        if ticker.active and ticker.ready then
            local speed = Toast.tickerSpeed or TICKERSPEED
            local containerWidth = ticker.scrollContainer:GetWidth() or 0
            
            local timeElapsed = GetTime() - ticker.scrollStartTime
            local distanceTraveled = speed * timeElapsed
            
            local activeFs = ticker.texts[ticker.activeStringIndex]
            local secondaryIndex = (ticker.activeStringIndex == 1) and 2 or 1
            local secondaryFs = ticker.texts[secondaryIndex]
            
            local activeX = containerWidth - distanceTraveled
            activeFs:ClearAllPoints()
            activeFs:SetPoint("LEFT", ticker.child, "LEFT", activeX, 0)
            
            local secondaryX = activeX + (activeFs.targetWidth or 0)
            secondaryFs:ClearAllPoints()
            secondaryFs:SetPoint("LEFT", ticker.child, "LEFT", secondaryX, 0)
            
            if activeX + (activeFs.targetWidth or 0) < 0 then
                ticker.activeStringIndex = secondaryIndex
                ticker.scrollStartTime = GetTime() - ((containerWidth - secondaryX) / speed)
                local hasMore = tickerLoadNextMessage(ticker.activeStringIndex == 1 and 2 or 1)
                
                if not hasMore and (secondaryX + (secondaryFs.targetWidth or 0) < 0) then
        ...
        if nBacklog >= MAXQUEUEBEFORETICKER then
            overflowActive = true
        elseif nBacklog <= (MAXQUEUEBEFORETICKER - 2) and not ticker.active then
...
local function buildToastDataFromV5(rec)
	...
	if not rec then 
        if pTime then L:ProfileStop("Toast:buildToastDataFromV5", pTime) end
        return nil 
    end
	local link = rec.il
	local texture
