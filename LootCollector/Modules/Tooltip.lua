...
local CACHE_SIZE_LIMIT = 400
local upgradeCacheQueue = {}
local wfCacheQueue = {}

local function AddToWFCache(itemID, isWF)
    if worldforgedCache[itemID] == nil then
        table.insert(wfCacheQueue, itemID)
        if #wfCacheQueue > CACHE_SIZE_LIMIT then
            local oldID = table.remove(wfCacheQueue, 1)
            worldforgedCache[oldID] = nil
        end
    end
    worldforgedCache[itemID] = isWF
end
...
local function IsWorldforged(tooltip, itemID, itemName)
    ...
    for i = 1, numLines do
        ...
        if string.find(lineText, "Worldforged", 1, true) then
...
local function AddUpgradeInfo(tooltip, upgradeChain)
    ...
    local numLines = tooltip:NumLines()
    for i = numLines, math.max(1, numLines - 10), -1 do
        local line = _G[tooltipName .. "TextLeft" .. i]
        local text = line and line:GetText()
        if text and text:find("^Upgrades:") then
            DebugPrint(">>> AddUpgradeInfo: Aborting, already applied to this tooltip.")
            return false
        end
    end
...
local function BuildUpgradeChain(baseItemID)
    ...
    if #newChain > 0 then
        if not upgradeCache[baseItemID] then
            table.insert(upgradeCacheQueue, baseItemID)
            if #upgradeCacheQueue > CACHE_SIZE_LIMIT then
                local oldID = table.remove(upgradeCacheQueue, 1)
                upgradeCache[oldID] = nil
            end
        end
...
local function HookTooltips()
    ...
    DebugPrint(printPrefix .. "Ready!")
end

local function ClearCache()
    wipe(upgradeCache)
    wipe(worldforgedCache)
    wipe(upgradeCacheQueue)
    wipe(wfCacheQueue)
...
function Tooltip:ApplySetting()
    ...
    if L and L.db and L.db.profile and not L.db.profile.enhancedWFTooltip then
        ClearCache()
    end
end
