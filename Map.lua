...
local function passesFilters(d)
  local f = getFilters(); if f.hideAll then return false end; local s = GetStatus(d); if s == STATUS_UNCONFIRMED and f.hideUnconfirmed then return false end; if s == STATUS_FADING and f.hideFaded then return false end; if s == STATUS_STALE  and f.hideStale  then return false end; if f.hideLooted and d and d.guid and isLootedByChar(d.guid) then return false end; local linkOrId = d.itemLink or d.itemID; if linkOrId then local _, _, quality, _, _, itemType, _, _, equipLoc = GetItemInfo(linkOrId); if quality and quality < (f.minRarity or 0) then return false end; if equipLoc and next(f.allowedEquipLoc) and not f.allowedEquipLoc[equipLoc] then return false end; if next(f.allowedClasses) then if itemType == "Armor" or itemType == "Weapon" then if isClassAgnosticItem(linkOrId) then else local playerClass = select(2, UnitClass("player")); if playerClass and not f.allowedClasses[playerClass] then return false end end end end end; return true
end
...
function Map:OpenPinMenu(anchorFrame)
    ...
    local name = d.itemLink or (select(1, GetItemInfo(d.itemID)) or "Discovery")
    ...
    table.insert(menuList, { text = "Set as looted", notCheckable = true, func = function() if not (L.db and L.db.char) then return end; L.db.char.looted = L.db.char.looted or {}; L.db.char.looted[d.guid] = time(); Map:Update() end })
    table.insert(menuList, { text = "Set as unlooted", notCheckable = true, func = function() if not (L.db and L.db.char and L.db.char.looted) then return end; L.db.char.looted[d.guid] = nil; Map:Update() end })
    
    table.insert(menuList, { text = "", notCheckable = true, disabled = true })

    
    table.insert(menuList, {
        text = "|cffff7f00Remove Discovery|r",
        notCheckable = true,
        func = function()
            StaticPopup_Show("LOOTCOLLECTOR_REMOVE_DISCOVERY", nil, nil, d.guid)
        end,
    })
