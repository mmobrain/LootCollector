...
local SLOT_OPTIONS = {
  { loc = "INVTYPE_HEAD",            text = "Head" },
  { loc = "INVTYPE_NECK",            text = "Neck" },
  { loc = "INVTYPE_SHOULDER",        text = "Shoulder" },
  { loc = "INVTYPE_CHEST",           text = "Chest" },
  { loc = "INVTYPE_ROBE",            text = "Robe" },
  { loc = "INVTYPE_WAIST",           text = "Waist" },
  { loc = "INVTYPE_LEGS",            text = "Legs" },
  { loc = "INVTYPE_FEET",            text = "Feet" },
  { loc = "INVTYPE_WRIST",           text = "Wrist" },
  { loc = "INVTYPE_HAND",            text = "Hands" },
  { loc = "INVTYPE_FINGER",          text = "Finger" },
  { loc = "INVTYPE_TRINKET",         text = "Trinket" },
  { loc = "INVTYPE_CLOAK",           text = "Back" },
  { loc = "INVTYPE_WEAPON",          text = "One-Hand" },
  { loc = "INVTYPE_2HWEAPON",        text = "Two-Hand" },
  { loc = "INVTYPE_WEAPONMAINHAND",  text = "Main Hand" },
  { loc = "INVTYPE_WEAPONOFFHAND",   text = "Off Hand" },
  { loc = "INVTYPE_SHIELD",          text = "Shield" },
  { loc = "INVTYPE_HOLDABLE",        text = "Held Off-hand" },
  { loc = "INVTYPE_RANGED",          text = "Ranged" },
  { loc = "INVTYPE_RANGEDRIGHT",     text = "Ranged (Right)" },
  { loc = "INVTYPE_THROWN",          text = "Thrown" },
  { loc = "INVTYPE_RELIC",           text = "Relic" },
}
...
local function GetStatus(d)
  ...
  if s == STATUS_FADING or s == STATUS_STALE or s == STATUS_CONFIRMED or s == STATUS_UNCONFIRMED then
    return s
  end
...
local function AlphaForStatus(status)
  if status == STATUS_FADING then return 0.65
  elseif status == STATUS_STALE then return 0.45 end
  return 1.0
end
...
local function isLootedByChar(guid)
  if not (L.db and L.db.char and L.db.char.looted) then return false end
  return L.db.char.looted[guid] and true or false
end

-- Helper: determine if item is class-agnostic for filtering
local function isClassAgnosticItem(linkOrId)
  -- Bags, consumables, trade goods, quest items, misc should not be hidden by class filters
  local _, _, _, _, _, itemType, _, _, equipLoc = GetItemInfo(linkOrId or "")
  if not itemType then return true end
  if itemType ~= "Armor" and itemType ~= "Weapon" then
    return true
  end
  -- Some Armor/Weapon types still all-class (e.g., cloaks for most classes); without tooltip parsing on 3.3.5, assume class filter applies only when explicitly chosen and equipLoc is present
  return not equipLoc or equipLoc == "" or equipLoc == "INVTYPE_CLOAK"
end
...
local function passesFilters(d)
  ...
  if linkOrId then
    local _, _, quality, _, _, itemType, _, _, equipLoc = GetItemInfo(linkOrId)
    if quality and quality < (f.minRarity or 0) then return false end
    if equipLoc and next(f.allowedEquipLoc) and not f.allowedEquipLoc[equipLoc] then
      return false
    end
    if next(f.allowedClasses) then
      -- Only apply class gate to equippable gear with likely class restrictions; allow others (e.g., bags) through
      if itemType == "Armor" or itemType == "Weapon" then
        if isClassAgnosticItem(linkOrId) then
          -- pass
        else
          local playerClass = select(2, UnitClass("player"))
          if playerClass and not f.allowedClasses[playerClass] then
            return false
          end
        end
      end
      -- Non-gear remains visible under class filters
    end
  end
...
function Map:GetDiscoveryIcon(d)
  ...
  if d and d.itemID then
    texture = select(10, GetItemInfo(d.itemID))
  end
  if (not texture) and d and d.itemLink then
    texture = select(10, GetItemInfo(d.itemLink))
  end
...
-- Render discovery tooltip (no left icon); if pinned, place a 16x16 hover icon directly after the header text
function Map:ShowDiscoveryTooltip(pin)
  if not pin or not pin.discovery then return end
  local d = pin.discovery

  GameTooltip:SetOwner(pin, "ANCHOR_RIGHT")
  GameTooltip:ClearLines()

  local name, _, quality = GetItemInfo(d.itemLink or d.itemID or "")
  local header = d.itemLink or name or "Discovery"
  GameTooltip:AddLine(header, 1, 1, 1, true)

  GameTooltip:AddLine(string.format("Found by: %s", d.foundBy_player or "Unknown"), 0.6, 0.8, 1, true)
  local ts = tonumber(d.timestamp) or time()
  GameTooltip:AddDoubleLine("Date", date("%Y-%m-%d %H:%M", ts), 0.8, 0.8, 0.8, 1, 1, 1)

  local status = GetStatus(d)
  local ls = tonumber(d.lastSeen) or ts
  GameTooltip:AddDoubleLine("Status", status, 0.8, 0.8, 0.8, 1, 1, 1)
  GameTooltip:AddDoubleLine("Last seen", date("%Y-%m-%d %H:%M", ls), 0.8, 0.8, 0.8, 1, 1, 1)

  if d.coords then
    GameTooltip:AddDoubleLine(
      "Location",
      string.format("%.1f, %.1f", (d.coords.x or 0) * 100, (d.coords.y or 0) * 100),
      0.8, 0.8, 0.8, 1, 1, 1
    )
  end
  ...
  -- Hover icon only when pinned; place immediately after the header text
  if self._pinnedPin == pin and (d.itemLink or d.itemID) then
    self:EnsureHoverButton()
    self._hoverBtnItemLink = d.itemLink or d.itemID
    local icon = self:GetDiscoveryIcon(d)
    self._hoverBtn.tex:SetTexture(icon or PIN_FALLBACK_TEXTURE)
    GameTooltip:Show() -- ensure header is created
    self._hoverBtn:ClearAllPoints()
    if GameTooltipTextLeft1 then
      self._hoverBtn:SetPoint("LEFT", GameTooltipTextLeft1, "RIGHT", 4, 0)
    else
      self._hoverBtn:SetPoint("TOPRIGHT", GameTooltip, "TOPRIGHT", -6, -6)
    end
    self._hoverBtn:Show()
  else
    if self._hoverBtn then self._hoverBtn:Hide() end
...
function Map:OpenPinMenu(anchorFrame)
  ...
  local name = d.itemLink or (GetItemInfo(d.itemLink or d.itemID or "") or "Discovery")

  wipe(menuList)
  table.insert(menuList, { text = tostring(name), isTitle = true, notCheckable = true })

  -- Per-character overlay actions
  table.insert(menuList, {
    ...
    func = function()
      ...
      L.db.char.looted[d.guid] = time()
      Map:Update()
    end
  })
  table.insert(menuList, {
    text = "Set as unlooted",
    notCheckable = true,
    func = function()
      if not (L.db and L.db.char and L.db.char.looted) then return end
      L.db.char.looted[d.guid] = nil
      Map:Update()
    end
  })
...
-- Build EasyMenu structure dynamically on click (rebuild to reflect live selections)
local function BuildFilterEasyMenu()
  ...
  local function addToggle(label, key)
    table.insert(menu, {
      ...
      keepShownOnClick = true,
      func = function() f[key] = not f[key]; Map:Update() end,
    })
  end

  addToggle("Hide All Discoveries", "hideAll")
  addToggle("Hide Looted", "hideLooted")
  addToggle("Hide Unconfirmed", "hideUnconfirmed")
  addToggle("Hide Faded", "hideFaded")
  addToggle("Hide Stale", "hideStale")
  addToggle("Show on Minimap", "showMinimap")

  -- Rarity submenu (refresh in place after selection)
  ...
  -- Slots submenu (multi-select)
  local slotsSub = {
    { text="Slots", isTitle=true, notCheckable=true },
    { text="Clear All", notCheckable=true, func=function() for k in pairs(f.allowedEquipLoc) do f.allowedEquipLoc[k]=nil end; Map:Update() end },
  }
  for _, opt in ipairs(SLOT_OPTIONS) do
    table.insert(slotsSub, {
      ...
      keepShownOnClick = true,
      func = function()
        if f.allowedEquipLoc[opt.loc] then
          f.allowedEquipLoc[opt.loc] = nil
        else
          f.allowedEquipLoc[opt.loc] = true
        end
        Map:Update()
      end,
    })
  ...
  -- Classes submenu (multi-select)
  local classesSub = {
    { text="Classes", isTitle=true, notCheckable=true },
    { text="Clear All", notCheckable=true, func=function() for k in pairs(f.allowedClasses) do f.allowedClasses[k]=nil end; Map:Update() end },
  }
  for _, classTok in ipairs(CLASS_OPTIONS) do
    ...
    table.insert(classesSub, {
      text = locName,
      checked = f.allowedClasses[classTok] and true or false,
      keepShownOnClick = true,
      func = function()
        if f.allowedClasses[classTok] then
          f.allowedClasses[classTok] = nil
        else
          f.allowedClasses[classTok] = true
        end
        Map:Update()
      end,
    })
  end
  table.insert(menu, { text="Classes", hasArrow=true, notCheckable=true, menuList=classesSub })

  return menu
end

-- UPDATED: Dynamically anchor the LC button to existing Blizzard or other addon UI elements.
local function PlaceFilterButton(btn)
    btn:ClearAllPoints()

    -- A list of potential anchor frames, in order of preference.
    -- We try to attach to other addon buttons first to avoid overlap.
    local potentialAnchors = {
        -- Addon-specific frames
        "_NPCScanOverlayWorldMapToggle",
    ...
    for _, anchorName in ipairs(potentialAnchors) do
        ...
        if anchorFrame and anchorFrame:IsShown() then
            -- Anchor our button to the LEFT of the found frame.
            btn:SetPoint("RIGHT", anchorFrame, "LEFT", -8, 0)
            return -- Found a suitable anchor, so we're done.
        end
    end

    -- Fallback if no preferred anchors are found. Position it near the top right.
...
function Map:EnsureFilterUI()
  ...
  if not FilterButton then
    ...
    FilterButton:SetScript("OnClick", function(self)
      if EasyMenu then
        EasyMenu(BuildFilterEasyMenu(), FilterMenuHost, self, 0, 0, "MENU", 2)
      else
        ToggleDropDownMenu(1, nil, FilterMenuHost, self, 0, 0)
      end
    end)
    -- Reposition the button whenever the map is shown, as other elements might have appeared/disappeared.
...
function Map:BuildPin()
  local pinSize = (L.db and L.db.profile.mapFilters.pinSize) or 16
  local frame = CreateFrame("Button", nil, WorldMapDetailFrame)
  frame:SetSize(pinSize, pinSize)
  frame:SetFrameLevel(WorldMapDetailFrame:GetFrameLevel() + 5)

  -- 1. Dark quickslot border BELOW everything else
  ...
  -- 2. Unlooted outline ABOVE border but BELOW icon
  frame.unlootedOutline = frame:CreateTexture(nil, "BORDER")
  frame.unlootedOutline:SetTexture("Interface\\Buttons\\WHITE8X8")
  frame.unlootedOutline:SetAllPoints(frame)
  frame.unlootedOutline:Hide()

  -- 3. Item icon ABOVE outline, inset to reveal the rim
  local ICON_INSET = 1
  frame.texture = frame:CreateTexture(nil, "ARTWORK")
  frame.texture:SetPoint("TOPLEFT", ICON_INSET, -ICON_INSET)
  frame.texture:SetPoint("BOTTOMRIGHT", -ICON_INSET, ICON_INSET)
...
local function EnsureMmPin(i)
  ...
  f:SetSize(Map._mmSize, Map._mmSize)
...
function Map:UpdateMinimap()
  local f = getFilters()
  if not f.showMinimap or not Minimap or (L.IsZoneIgnored and L:IsZoneIgnored()) then self:HideAllMmPins(); return end
  local mapID = GetCurrentMapZone()
  local px, py = GetPlayerMapPosition("player")
  if not px or not py or (px == 0 and py == 0) then self:HideAllMmPins(); return end
  local count = 0
  local centerX, centerY = Minimap:GetWidth() * 0.5, Minimap:GetHeight() * 0.5
  local radius = math.min(centerX, centerY) - 6
  local facing = GetRotateMinimapFacing()
  for _, d in pairs(L.db.global.discoveries or {}) do
    repeat
      if not d or not d.coords or not d.zoneID or d.zoneID ~= mapID or not passesFilters(d) then break end
      local dx, dy = (d.coords.x or 0) - px, (d.coords.y or 0) - py
      local theta = math.atan2(dx, -dy) or 0
      theta = theta - facing
      count = count + 1
      local pin = EnsureMmPin(count)
      pin.discovery = d
      local x, y = centerX + math.cos(theta) * radius, centerY + math.sin(theta) * radius
      pin:ClearAllPoints(); pin:SetPoint("CENTER", Minimap, "BOTTOMLEFT", x, y)
      local icon = self:GetDiscoveryIcon(d)
      pin.tex:SetTexture(icon or PIN_FALLBACK_TEXTURE)
      pin:Show()
    until true
  end
  for i = count + 1, #self._mmPins do self._mmPins[i]:Hide(); self._mmPins[i].discovery = nil end
end
...
function Map:EnsureMinimapTicker()
  ...
  self._mmTicker:SetScript("OnUpdate", function(_, elapsed)
    Map._mmElapsed = Map._mmElapsed + elapsed
    if Map._mmElapsed >= Map._mmInterval then Map._mmElapsed = 0; Map:UpdateMinimap() end
  end)
end

function Map:OnInitialize()
  if not (L.db and L.db.profile and L.db.global and L.db.char) then return end
  -- Defaults are set centrally by LootCollector.lua now
  self:EnsureFilterUI()
  self:EnsureMinimapTicker()
  if WorldMapDetailFrame then
    WorldMapDetailFrame:EnableMouse(true)
    WorldMapDetailFrame:SetScript("OnMouseDown", function() if Map._pinnedPin then Map._pinnedPin = nil; Map:HideDiscoveryTooltip() end end)
  end
  if WorldMapFrame and hooksecurefunc then
    hooksecurefunc(WorldMapFrame, "Hide", function() if Map._pinnedPin then Map._pinnedPin = nil; Map:HideDiscoveryTooltip() end end)
  end
end
...
function Map:Update()
  ...
  local filters = (L.db.profile and L.db.profile.mapFilters) or {}
  ...
  local pinIndex = 1
  local stillPinned = false

  for _, discovery in pairs(L.db.global.discoveries) do
    repeat
      if not discovery or not discovery.coords or not discovery.zoneID or discovery.zoneID ~= mapID or not passesFilters(discovery) then break end
      local pin = self.pins[pinIndex] or self:BuildPin()
      pinIndex = pinIndex + 1
      pin.discovery = discovery

      local pinSize = (L.db.profile.mapFilters.pinSize) or 16
      pin:SetSize(pinSize, pinSize)

      local isLooted = isLootedByChar(discovery.guid)
      if isLooted then
          pin.unlootedOutline:Hide()
          pin.texture:SetVertexColor(0.7, 0.7, 0.7)
      else
          pin.unlootedOutline:Show()
          pin.texture:SetVertexColor(1, 1, 1)
          local _, _, quality = GetItemInfo(discovery.itemLink or discovery.itemID)
          local r, g, b = GetQualityColor(quality)
      ...
      pin:SetAlpha(AlphaForStatus(GetStatus(discovery)))

      local x, y = (discovery.coords.x or 0) * mapWidth, (discovery.coords.y or 0) * mapHeight
      pin:ClearAllPoints(); pin:SetPoint("CENTER", WorldMapDetailFrame, "TOPLEFT", x, -y)
      pin:Show()
